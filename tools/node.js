...
new Function("domprops", "exports", function() {
    let code = exports.FILES.map(function(file) {
...
function describe_ast() {
    let out = OutputStream({ beautify: true });
    doitem(AST_Node);
    return out.get() + "\n";

    function doitem(ctor) {
        out.print("AST_" + ctor.TYPE);
        let props = ctor.SELF_PROPS.filter(function(prop) {
            return !/^\$/.test(prop);
        });
        if (props.length > 0) {
            out.space();
...
function infer_options(options) {
    let result = exports.minify("", options);
    return result.error && result.error.defs;
}

exports.default_options = function() {
    let defs = infer_options({ 0: 0 });
    Object.keys(defs).forEach(function(component) {
        let options = { module: false };
